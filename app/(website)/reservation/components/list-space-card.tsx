...
// Descriptions des appartements (inchangées)
...
// Composant pour afficher les détails de l'appartement
function ApartmentDetails({ formRef }: { formRef: React.RefObject<HTMLFormElement> }) {
  ...
  React.useEffect(() => {
    if (!apartment) {
      router.push("/"); // Redirige vers la page d'accueil si aucun appartement n'est spécifié
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-1/2">
          ...
          <p className="text-amber-600 leading-relaxed">
            {apartmentDescriptions[apartment] || "Découvrez cet espace moderne conçu pour offrir une expérience de vie exceptionnelle."}
          </p>
        </div>
      </div>
    </div>
...
export default function RoomReservationForm({
  ...
  const calculateAmount = (guests: number, roomType: string, dates: Date[]): number => {
  ...
  }, [guests, roomType, selectedDates]);

  // Gestion de la soumission du formulaire
  const onSubmit: SubmitHandler<IFormInput> = async (formData) => {
    setIsSubmitting(true);
    try {
      // Redirection simplifiée avec les données du formulaire
      router.push(
        `/recap?type=reservation&name=${encodeURIComponent(formData.name)}&email=${encodeURIComponent(formData.email)}&phone=${encodeURIComponent(formData.phone)}&hotelId=${hotel.id}&dates=${encodeURIComponent(selectedDates.map((date) => dayjs(date).format("YYYY-MM-DD")).join(","))}&guests=${guests}&roomType=${roomType}&amount=${totalAmount}&apartment=${encodeURIComponent(apartment || "")}`
      );
    } catch (error) {
          ...
          <CardContent className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <p className="text-lg font-bold text-amber-800">
                {totalAmount > 0
                  ? `${totalAmount} FCFA (${sortedDates.length} nuit${sortedDates.length > 1 ? "s" : ""})`
                  : "Non calculé"}
              </p>
            </div>
            <Button
